...
export function IQVisionConfigPanel() {
  ...
  const endpointQuery = useQuery({
    queryKey: ["iqvision", "endpoint", selectedSiteId],
    queryFn: () => getIQVisionEndpoint(selectedSiteId!),
  ...
  const [timeWindow, setTimeWindow] = useState<string>("weektodate");
  ...
  const pointsQuery = useQuery({
    queryKey: ["iqvision", "points", selectedSiteId],
    queryFn: () => listIQVisionPoints(selectedSiteId!),
    enabled: !!selectedSiteId && !!endpointQuery.data,
  });

  const saveMut = useMutation({
    mutationFn: () =>
      putIQVisionEndpoint(selectedSiteId!, {
    ...
    onSuccess: () => {
      ...
      setPassword("");
      queryClient.invalidateQueries({ queryKey: ["iqvision", "endpoint", selectedSiteId] });
    },
    onError: (e: Error) => setStatusMsg(`Save failed: ${e.message}`),
  });

  const deleteMut = useMutation({
    mutationFn: () => deleteIQVisionEndpoint(selectedSiteId!),
    onSuccess: () => {
      setStatusMsg("Endpoint removed.");
      queryClient.invalidateQueries({ queryKey: ["iqvision", "endpoint", selectedSiteId] });
      queryClient.invalidateQueries({ queryKey: ["iqvision", "points", selectedSiteId] });
    },
    onError: (e: Error) => setStatusMsg(`Delete failed: ${e.message}`),
  });

  const testMut = useMutation({
    mutationFn: () => testIQVisionEndpoint(selectedSiteId!),
    onSuccess: (r) => setStatusMsg(`Reachable (HTTP ${r.status_code}).`),
    onError: (e: Error) => setStatusMsg(`Unreachable: ${e.message}`),
  });

  const scanMut = useMutation({
    mutationFn: () => startIQVisionScan(selectedSiteId!),
  ...
  const syncMut = useMutation({
    mutationFn: () => startIQVisionSync(selectedSiteId!, timeWindow),
  ...
  const pointsByEquip = useMemo(() => {
    const groups = new Map<string, IQVisionScanPoint[]>();
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center text-sm text-muted-foreground">
          Select a site from the top bar to configure its IQVision endpoint.
        </CardContent>
      </Card>
    );
  }
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <p className="text-sm font-normal text-muted-foreground">
            Per-site IQVision station credentials. Same BQL scan + history shape as Niagara;
            points are grouped into equipment by the BQL <code className="rounded bg-muted px-1 text-xs">Device</code> column.
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <div>
              <label htmlFor="iqvision-base-url" className="mb-1 block text-xs font-medium text-muted-foreground">
                Base URL
              </label>
              <input
                id="iqvision-base-url"
                className={`${field} w-full`}
                value={baseUrl}
                onChange={(e) => setBaseUrl(e.target.value)}
                placeholder="https://iqvision.local"
              />
            </div>
            <div>
              <label htmlFor="iqvision-username" className="mb-1 block text-xs font-medium text-muted-foreground">
                Username
              </label>
              <input
                id="iqvision-username"
                className={`${field} w-full`}
                value={username}
            ...
            <div>
              <label htmlFor="iqvision-password" className="mb-1 block text-xs font-medium text-muted-foreground">
                Password {hasEndpoint && <span className="text-muted-foreground/60">(leave blank to keep current)</span>}
              </label>
              <input
                id="iqvision-password"
                type="password"
                className={`${field} w-full`}
                value={password}
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            ...
            {hasEndpoint && (
              <button
                type="button"
                onClick={() => {
                  if (window.confirm("Remove the IQVision endpoint for this site?")) {
                    deleteMut.mutate();
                  }
                }}
      ...
      {hasEndpoint && (
        <Card>
          <CardHeader className="pb-2">
            ...
            <p className="text-sm font-normal text-muted-foreground">
              Grouped by equipment from the BQL <code className="rounded bg-muted px-1 text-xs">Device</code> column
              (<code className="rounded bg-muted px-1 text-xs">proxyExt.device.displayName</code>). A tag value
