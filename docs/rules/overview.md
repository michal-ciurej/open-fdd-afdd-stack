...
Config: `rules_dir: "stack/rules"` locally (GET/PUT `/config` or `OFDD_RULES_DIR` at bootstrap). **On Azure set `OFDD_RULES_DIR=/app/config/rules`** (absolute, matching the `OFDD_BRICK_TTL_PATH=/app/config/data_model.ttl` convention) — a directory on the shared `predmain-config` Azure Files mount (mounted at `/app/config`) that the API container and both jobs read/write, so there is **one** authoritative copy (no image-vs-DB drift; edits are picked up without rebuilding an image). Use the absolute path so resolution never depends on the container's working directory. If the configured path does not exist the loop falls back to the baked-in `stack/rules`; and an *empty* `/app/config/rules` is seeded once from those baked-in defaults on first boot (`ensure_rules_dir_seeded`), then left authoritative. Default rules ship in `stack/rules/` (e.g. `sensor_bounds.yaml`, `sensor_flatline.yaml`). See the [Expression Rule Cookbook](../expression_rule_cookbook) to add or adapt rules. For how YAML becomes pandas operations and how telemetry is pivoted into a DataFrame, see [YAML rules → Pandas (under the hood)](pandas_yaml_dataframes).
