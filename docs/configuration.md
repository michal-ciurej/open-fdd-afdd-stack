...
| `rules_dir` | stack/rules (local) / **/app/config/rules (Azure)** | **Single directory for your rules** (hot reload). On Azure this is the shared `predmain-config` mount so the API and both jobs share one authoritative copy — set `OFDD_RULES_DIR=/app/config/rules` (**absolute**, like `OFDD_BRICK_TTL_PATH=/app/config/data_model.ttl`, so it always lands on the mount). An empty `/app/config/rules` is seeded once from the baked-in `stack/rules` defaults on first boot. |
