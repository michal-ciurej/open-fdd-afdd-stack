...
**File handoff (`openclaw/issues_log.md`)** - [`HANDOFF_PROTOCOL.md`]( /open-fdd-afdd-stack/tree/main/openclaw/HANDOFF_PROTOCOL.md) assumes both Cursor and OpenClaw can read the **same repo tree**. With a split setup, use **git push/pull** (or SSH to one canonical clone) so the lab trail stays in sync.
