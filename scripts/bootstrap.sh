...
run_bootstrap_doctor() {
  ...
  if [[ -d "$sibling/.git" ]] || [[ -f "$sibling/bacpypes_server/main.py" ]]; then
    ...
    echo "[WARN] diy-bacnet-server not found beside this repo (bootstrap will clone if needed): $sibling"
  fi
...
ensure_diy_bacnet_sibling() {
  ...
  echo "=== Cloning diy-bacnet-server (sibling of open-fdd-afdd-stack) ==="
...
verify_code() {
  ...
  local pytest_target="openfdd_stack/tests/"
  if [[ "$MODE" == "collector" ]]; then
    pytest_target="openfdd_stack/tests/platform/test_bacnet_driver.py openfdd_stack/tests/platform/test_bacnet_api.py"
  elif [[ "$MODE" == "model" ]]; then
    pytest_target="openfdd_stack/tests/platform/test_data_model_api.py openfdd_stack/tests/platform/test_data_model_ttl.py openfdd_stack/tests/platform_api/test_model_context.py"
  elif [[ "$MODE" == "engine" ]]; then
    # FDD-loop–oriented tests (no ha_integration suite in this repo)
...
if $UPDATE_PULL_REBUILD; then
  ...
  if [[ -d "$REPO_ROOT/.git" ]]; then
    echo "Pulling open-fdd-afdd-stack..."
    (cd "$REPO_ROOT" && git pull --rebase 2>/dev/null || git pull 2>/dev/null) || true
  else
    echo "Skipping stack git pull (not a git repo)."
  fi
