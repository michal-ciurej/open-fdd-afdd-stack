...
def test_auth_required_when_api_key_set():
    with patch("openfdd_stack.platform.api.auth.get_platform_settings") as m:
        s = m.return_value
        s.api_key = "secret123"
        r = client.get("/capabilities")
...
def test_error_schema_on_404():
    ...
    with patch("openfdd_stack.platform.api.points.get_conn") as mock_get_conn:
        conn = MagicMock()
        cur = MagicMock()
...
def test_config_returns_platform_config():
    with patch("openfdd_stack.platform.api.config.get_config_overlay", return_value=None):
        with patch(
            "openfdd_stack.platform.api.config.get_config_from_graph",
